...
const RevenueChart: FC<RevenueData> = ({data}) => {
  ...
  return (
    <div>
      <div className="bg-[#F5F2F3] p-4 w-full max-w-sm">
        {/* Header */}
        <div className="flex justify-between items-center mb-4 border-b border-gray-500">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-[#0C4B14] rounded-sm"></div>
        ...
        <div className="flex gap-2 mb-2">
          <span className="flex text-xl font-bold gap-1 text-gray-900">
            <p className="text-md">{data.currency}</p>
          ...
          <div className="flex items-center pt-1 gap-1 text-sm text-green-500">
            ...
            >
              <path d="m132 1068v-1068h-132v1200h1200v-132z" fill="#9BF22B" />
              <path
                d="m852.37 283.31 91.898 70.582-191.73 230.98-215.12-170.7-307.74 358.97 77.062 66.07 244.04-284.78 216.27 171.68 257.56-309.61 91.391 71.074v-270.78z"
                fill="#47d754"
              />
            </svg>
            <p className="text-xs"> +{data.percentage_change || 0}</p>{" "}
            <p className="text-gray-500 ml-1 text-xs">from last month</p>
          </div>
        </div>
      </div>
      <ChartContainer config={chartConfig} className="h-40 max-xl:w-48 bg-[#F5F2F3]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} barCategoryGap="20%" barGap={2}>
            ...
            <Bar dataKey="revenue" fill="#0C4B14" radius={4} />
            <Bar dataKey="expense" fill="#9BF22B" radius={4} />
          </BarChart>
        </ResponsiveContainer>
      </ChartContainer>
    </div>
